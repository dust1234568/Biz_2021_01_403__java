...
public class ClassServiceV2 {
	...
	// 인스턴스 변수
	// ClassServiceV2 클래스를 사용하여 (어디선가) 객체를 선언하고 초기화하여
	// 인스턴스화 되었을 때 부터 자동으로 생성되는 변수

	// - 현재 클래스의 모든 method에서 공용으로 읽고 쓰기가 가능한 변수들
	private Random rnd = new Random(); // method와 같다
	private int sum = 0;
	...
	public void for_1() {
		...
		// sum1 = (method) 지역변수
		// for_1() method 가 끝나면 소멸
		int sum1 = 0; // method 변수
		...
		for (int i = 0; i < 10; i++) {
			...
			// num : (for반복문) 지역변수
			// for(int...) 명령문이 끝나면 소멸
			int num = i + 1; // for()이 끝나면 사라짐, 지역변수
			sum1 += num;
		}
	...
	public void input() {
		 ...
		 * Scanner 클래스를 객체로 만들고 사용이 끝나면 
		 * 연결을 해제(끊는)하는 과정이 계속 반복된다
		 */
	...
	// 나눗셈
	// 인스턴스 변수로 만들어두고 사용이 편리
	public void division() {
		/*
		 * Scanner를 scan 인스턴스변수로 선언
		 * 
		 * 현재 클래스(Css..V2)가 객체로 선언되고 인스턴스로 만들어 질때 1번만 키보드에 연결된다
