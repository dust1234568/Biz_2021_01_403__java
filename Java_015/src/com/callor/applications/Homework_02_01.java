...
public class Homework_02_01 {
	...
	/*
	 * 2 이상의 임의의 정수 100개를 만들고 PrimeServiceV2의 prime() method 호출하여 임의 정수 100개 중 소수인
	 * 수들의 리스트를 출력(배열 이용) 소수인 수들의 합
	 */
	public static void main(String[] args) {
		 ...
		 * 
		 * 통상적으로 코딩을 할때 변수, 객체등의 선언 코드는 가급적 코드의 시작부분(클래스 선언 명령문 아래 , method 선언문 아래)에 작성을
		 * 한다
		 */
		...
		// 생성된 rndNums 배열에 임의 정수100개를 저장해 두었다
		// 이처럼 배열을 만들어두면 이후의 코드에서
		// 한번 생성된 임의 정수 100개를 자유롭게 활용할 수 있다.
		for (int i = 0; i < rndNums.length; i++) {
			rndNums[i] = rnd.nextInt(10000) + 2;
		}

		// rndNums 배열에 담겨있는 정수들을
		// psV2.prime() method에게 보내서 소수인가를 검사하자

		// 다음처럼 prime() method에 배열을 통째로 전달하여
		// 코드를 수행할 수 없는 이유
		// psV2.prime(rndNums); -> 두개의 type이 달라서 안된다
