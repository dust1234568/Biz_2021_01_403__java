...
public class CartServiceImplV2 implements CartService {
	 ...
	 * 
	 * 3. NullPointerException을 방지하기 위하여 클래스의 생성자 method에
	 *  객체변수를 초기화 하는 코드를 반드시 추가해 준다
	 *  
	 * 즉, 인스턴스 객체변수 선언 -> 생성자에서 객체변수 초기화(생성)
	 *  
	 * 이렇게 복잡한 과정으로 코드를 작성하는 이유는 
	 * Java GC(Garbage Collection : 메모리 자동 관리)가 작동되는 과정에서
	 * 혹시 모를 메모리 누수(leak)를 방지 하기 위한 조치
	 *  
	 * Java GC : 프로젝트가 실행되는 과정에서 사용이 끝났음에도
	 * 아직 메모리에 남아 있는 변수, 객체 등을 자동으로 제거하여
	 ...
	 * 
	 * 클래스를 객체로 선언하고, 초기화(생성)를 할때 사용하는 아래 코드에서
	 * CartServiceimplV2 cs = new CartServiceImplV2();
	 * 
	 * new CartServiceImplV2() 이 부분의 코드가 클래스의 생성자를 호출하여 
	 * 객체를 생성하는 부분이다
	 * 
	 * 생성자 method는 특별한 경우가 아니면 클래스를 선언할때 자동으로 코드가
	 * 만들어지고 감춰진 상태로 존재한다
	 */
