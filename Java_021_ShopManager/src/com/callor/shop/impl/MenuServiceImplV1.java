...
public class MenuServiceImplV1 implements MenuService {
	...
	public Integer selectMenu() {
		...
		while (true) {
			...
			// 메뉴를 선택할 수 있도록 입력 프롬프트 생성
			System.out.print("입력 >> ");
			// 문자열로 입력 받기 
			String strMenu = scan.nextLine();

			// equals는 내용 자체를 비교, == 는 주소값을 비교
			// 입력된 값이 문자열 "QUIT" 과 같으면 업무종료 문구를 출력하고 break 
			if (strMenu.equals("QUIT")) { 
				System.out.println("업무종료");
				break;
			}
			// 입력된 문자열을 정수형으로 형변형시키고 그때 발생할수 있는 exception을 막기위한 유효성검사 를 실시한다
			try {
				int intMenu = Integer.valueOf(strMenu);
				// 입력된 문자열을 정수형으로 형변환시켜서 그 값이 1 이상 3 이하 일 경우 intMenu로 값이 return된다
				...
				// QUIT와 1 ~ 3 까지 정수 외의 문자열과 정수 값을 입력 할 경우 아래 출력문을 출력
			} catch (Exception e) {
